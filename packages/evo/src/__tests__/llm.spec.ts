...
const think = async (goal: string) => {
  ...
  //   ids: ["1", "2"],
  //   metadatas: [{ action: "writeFile" }, { action: "executeScript" }],
  //   documents: [
  //     "write 'hello world!' in a file called foo.txt",
  //     "run the python script called foo.py",
  //   ],
  // });

  // console.log({ addResponse });

  // const documentsAgain = await collection.get({
  //   whereDocument: { $contains: "script" },
  // });
  // console.log(documentsAgain);
};
...
describe("LLM Test Suite", () => {
  ...
  test(`Execute`, async () => {
    ...
    //     chat,
    //     currentFunctions.map((f) => f.definition)
    //   );
